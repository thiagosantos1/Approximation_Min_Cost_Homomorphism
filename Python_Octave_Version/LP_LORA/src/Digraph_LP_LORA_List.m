...
function result = get_first_out_neighbor(vertex, matrix) 
  result =0;
...
# function to return the l(i) of a vertex
# get the first neighbor of a vertex
function result = get_first_in_neighbor(vertex, matrix) 
  result =0;
  for vertex_pair = 1:length(matrix); 
  ...
  # then for each pair, you compare with all vertex in right of G, if there is an edge
    for vertex_g_v = vertices_graph_g;
      if adj_matrix_g(vertex_g_u,[vertex_g_v]) ==1; # if there is edge
        in_nbr_i = get_first_in_neighbor(vertex_h_i,adj_matrix_h); # get the first neighbor of v
        if in_nbr_i >0;
          size_A = size(A);
...
for vertex_g_u = vertices_graph_g; 
  ...
  # then for each pair, you compare with all vertex in right of G, if there is an edge
    for vertex_g_v = vertices_graph_g;
      if adj_matrix_g(vertex_g_u,[vertex_g_v]) ==1; # if there is edge
        out_nbr_i = get_first_out_neighbor(vertex_h_i,adj_matrix_h); # get the first neighbor of v
        if out_nbr_i > 0;
          size_A = size(A);
          A(size_A(1) +1, [adj_matrix_index_saving(vertex_g_u,[vertex_h_i] )] )= -1;
