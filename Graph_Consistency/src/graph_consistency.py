...
class Graph:
  ...
  def __init__(self, file_name,type_graph):
    ...
    self.path = ""
    self.sub_path = "/../../LP_LORA/src/" # to save the file in the LP folder automatic
  ...
  def save_list_homomophism(self):
    try:
      ...
      list_output_local = open(self.list_output_name+".txt", 'w')
    except IOError as exc:
    ...
    for key,value in sorted(self.list_match_G_H.items()):
      ...
      for x in range(len(value)):
        ...
        list_output_local.write(str(value[x]-1))
        if x < len(value) -1:
          list_output.write(" ")
          list_output_local.write(" ")
      if size >0:
        list_output.write("\n")
        list_output_local.write("\n")
  ...
  def save_list_pairs(self):
    try:
      ...
      list_output_local = open(self.list_pair_out_name+".txt", 'w')
    except IOError as exc:
    ...
    for key,value in graph_g.list_pairs.items():
      ...
      list_output_local.write(str(list_pairs[0] -1) + " " + str(list_pairs[1]-1) + "\n") # key in one line
      size_value = len(value)
      for pair in value:
        size_value -=1
        xy = get_x_y_hash(pair, len(graph_h.vertices))
        list_output.write(str(xy[0]) + " " + str(xy[1]))
        list_output_local.write(str(xy[0] -1) + " " + str(xy[1]-1))
        if size_value >0:
          list_output.write(" ")
          list_output_local.write(" ")
